...
<form method="POST" action="{{@$move_url}}/1" id="frm_{{$special_id}}">
	{!! csrf_field() !!}
	...
	{{-- <input type="hidden" name="submit_type" value="1" id="submit-{{$special_id}}" /> --}}
	<input name='year_to' type="hidden" />
</form>
<p class="form-group">
   	{{-- <div class="btn-group dropup">
		...
		<ul class="dropdown-menu">
			<li>
				<a href="{{@$export_url}}">{{trans('general.all')}}</a>
			</li>
			<li>
				<a href="javascript:void(0);" onclick="$('#submit-{{$special_id}}').val(1);$('#frm_{{$special_id}}').submit();">
					{{trans('general.selected_records')}}
				</a>
			</li>
			<li>
				<a href="javascript:void(0);" onclick="$('#submit-{{$special_id}}').val(2);$('#frm_{{$special_id}}').submit();">{{trans('general.all_current_filter_records')}}</a>
			</li>
		</ul>
	</div> --}}

	@if (isset($extra_dropdown) && (empty($status) || $status != 1))
	<div class="btn-group dropup">
		<button class="btn btn-default" type="button">
			{{trans('general.move_selected_records_to')}}
		</button>
		<button class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
			<span class="caret"></span>
		</button>
		<ul class="dropdown-menu">
			@foreach ($extra_dropdown as $r)
			<li>
				<a href="javascript:void(0);" 
					onclick="$('#move_{{$special_id}} [name=\'year_to\']').val({{$r['year']}});$('#move_{{$special_id}}').submit()">
					{{$r['title']}}
				</a>
			</li>
			@endforeach
	...
	<div class="btn-group dropup">
		<button class="btn btn-default" type="button">
			{{trans('general.move_filtered_records_to')}}
		</button>
		<button class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
			<span class="caret"></span>
		</button>
		<ul class="dropdown-menu">
			...
			<li>
				<a href="javascript:void(0);" 
					onclick="$('#frm_{{$special_id}} [name=\'year_to\']').val({{$r['year']}});$('#frm_{{$special_id}}').submit()">
					{{$r['title']}}
				</a>
			</li>
			@endforeach
...
<form method="POST" action="{{@$move_url}}/0" id="move_{{$special_id}}">
	{!! csrf_field() !!}
	<input name='year_to' type="hidden" />
</form>
	...
	$(document).ready(function () {
	   	...
	   	function filterUpdate(table_id)
	    {
	   	...
	   	$('#frm_{{$special_id}}').on('submit', function(e){
	      	...
	      	// if (form.submit_type.value == 1 && rows_selected.length == 0) {
