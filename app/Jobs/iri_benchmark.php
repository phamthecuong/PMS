...
class iri_benchmark implements ShouldQueue
{
    ...
    public function handle()
    {
        try
        {
            ...
            $log = [$deterioration->log_text, date('Y-m-d H:i:s') . ': [iri] update benchmark flg to ' . ($deterioration->benchmark_flg + 1)];
            $deterioration->log_text = implode(PHP_EOL , $log);
            $deterioration->benchmark_flg += 1;
            $deterioration->save();

            $iri_pavement_type_phi = (new \App\Jobs\iri_phi($this->session_id))->onQueue('deterioration_iri');
            dispatch($iri_pavement_type_phi);
        }
        catch (\Exception $e)
        {
            \Log::info('iri_benchmark');
