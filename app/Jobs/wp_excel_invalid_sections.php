...
class wp_excel_invalid_sections extends WorkPlanningExcel implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try
        {
            ...
            $xml = $this->_writeSheet($this->session_id, 1, $xml, $list, 'en');
            $xlsx->arrXMLs['/xl/worksheets/sheet6.xml'] = $xml;
            $xlsx->Output("Output_WP_". $list . "_en.xlsx", "F");

            $template = "Output_WP_". $list . "_vn.xlsx";
            $xlsx = new \eiseXLSX($template);
            $xml = $xlsx->arrXMLs['/xl/worksheets/sheet6.xml'];
            $xml = $this->_writeSheet($this->session_id, 1, $xml, $list, 'vn');
            $xlsx->arrXMLs['/xl/worksheets/sheet6.xml'] = $xml;
            $xlsx->Output("Output_WP_". $list . "_vn.xlsx", "F");

            $work_planning = tblWorkPlanning::findOrFail($this->session_id);
            $work_planning->{"excel_flg_{$list}"} = 6;
            $work_planning->save();

            $work_planning_excel = (new \App\Jobs\wp_excel_planned_sections($this->session_id, $list))->onQueue('work_planning_excel');
            dispatch($work_planning_excel);
        }
        catch (\Exception $e)
        {
            \Log::info('work_planning_excel_6');
