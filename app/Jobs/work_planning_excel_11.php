...
class work_planning_excel_11 extends WorkPlanningExcel implements ShouldQueue
{
    ...
    public function handle()
    {
        try
        {
            ...
            $xml = $this->_writeSheet($this->session_id, 6, $xml);
            $xlsx->arrXMLs['/xl/worksheets/sheet12'] = $xml;
            $xlsx->Output("Output_WP.xlsx", "F");
            $work_planning = tblWorkPlanning::findOrFail($this->session_id);
            $work_planning->excel_flg = 10;
            $work_planning->save();
            $work_planning_excel = (new \App\Jobs\work_planning_excel_12($this->session_id))->onQueue('work_planning_excel');
