...
class wp_excel_clist3 extends WorkPlanningExcel implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try
        {
            ...
            $xml = $xlsx->arrXMLs['/xl/worksheets/sheet11.xml'];
            $xml = $this->_writeSheet($this->session_id, 6, $xml, $list, 'en');
            $xlsx->arrXMLs['/xl/worksheets/sheet11.xml'] = $xml;
            $xlsx->Output("Output_WP_". $list ."_en.xlsx", "F");

            $template = "Output_WP_". $list ."_vn.xlsx";
            $xlsx = new \eiseXLSX($template);
            $xml = $xlsx->arrXMLs['/xl/worksheets/sheet11.xml'];
            $xml = $this->_writeSheet($this->session_id, 6, $xml, $list, 'vn');
            $xlsx->arrXMLs['/xl/worksheets/sheet11.xml'] = $xml;
            $xlsx->Output("Output_WP_". $list ."_vn.xlsx", "F");

            $work_planning = tblWorkPlanning::findOrFail($this->session_id);
            $work_planning->{"excel_flg_{$list}"} = 10;
            $work_planning->save();
            $work_planning_excel = (new \App\Jobs\wp_excel_clist4($this->session_id, $list))->onQueue('work_planning_excel');
            dispatch($work_planning_excel);
        }
        catch (\Exception $e)
        {
            \Log::info('work_planning_excel_11');
