...
class crack_epsilon implements ShouldQueue
{
    ...
    public function handle()
    {
        try
        {
            ...
            $crack_21 = (new \App\Jobs\crack_21($this->session_id))->onQueue('deterioration_crack');
            dispatch($crack_21);
        }
        catch (\Exception $e)
        {
            \Log::info('crack_epsilon');
    ...
    private function _writeExcel()
    {
        ...
        foreach($option[strtolower($link)]  as $row)
        {
            $to =  $row['to'];
            $from =  $row['from'];
            $array = Helper::convertConditionInforToText($from, $to, 'C');
            $year[] = $array;
        }
    ...
    private function createChartDataPerTypeColor($xlsx, $name, $case, $from, $max)
    {
        $chart_name = $name;
        $xml = simplexml_load_string($xlsx->arrXMLs['/xl/charts/' . $chart_name . '.xml']);
        $xml = $this->setEdit($xml,$case);
        return $xml;
    }
    private function _sxml_append(\SimpleXMLElement $to, \SimpleXMLElement $from) 
    {
     ...
     private function _sxml_edit(\SimpleXMLElement $to, \SimpleXMLElement $from) 
    {
        $toDom = dom_import_simplexml($to);
        $fromDom = dom_import_simplexml($from);
        $toDom->nodeValue = $fromDom;
    }
    private function setEdit($xml, $case)
    {
        $color = array("FF3333", "FFC233",'AEFF33', '33FF47', '33FFD7', '3399FF',"5C33FF", "FF3333", "FFC233",'AEFF33', '33FF47', '33FFD7', '3399FF',"5C33FF", "FF3333", "FFC233",'AEFF33', '33FF47', '33FFD7', '3399FF',"5C33FF");
        for ($i=0; $i < $case ; $i++) { 
            $scaling = $xml->children('c', TRUE)->chart->children('c', TRUE)->plotArea->children('c', TRUE)->areaChart->children('c', TRUE)->ser[$i]->children('c', TRUE)->spPr->children('a', TRUE)->solidFill->children('a', TRUE)->srgbClr;
            $data = dom_import_simplexml($scaling);
            $data->setAttribute('val',$color[$i]);
        }
        return $xml;
    }
    private function setMaxXAxis($xml, $maximum_x_axis)
    {
        ...
        $this->_sxml_append($scaling, simplexml_load_string('<c:max val="' . $maximum_x_axis . '"/>'));
        return $xml;
    }
