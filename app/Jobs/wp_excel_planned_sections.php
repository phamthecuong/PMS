...
class wp_excel_planned_sections extends WorkPlanningExcel implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try
        {
            ...
            $xml = $xlsx->arrXMLs['/xl/worksheets/sheet7.xml'];
            $xml = $this->_writeSheet($this->session_id, 11, $xml, $list, 'en');
            $xlsx->arrXMLs['/xl/worksheets/sheet7.xml'] = $xml;
            ...
            $xlsx = new \eiseXLSX($template);
            $xml = $xlsx->arrXMLs['/xl/worksheets/sheet7.xml'];
            $xml = $this->_writeSheet($this->session_id, 11, $xml, $list, 'vn');
            $xlsx->arrXMLs['/xl/worksheets/sheet7.xml'] = $xml;
            $xlsx->Output("Output_WP_". $list ."_vn.xlsx", "F");

            $work_planning = tblWorkPlanning::findOrFail($this->session_id);
            $work_planning->{"excel_flg_{$list}"} = 7;
            $work_planning->save();

            $work_planning_excel = (new \App\Jobs\wp_excel_clist1($this->session_id, $list))->onQueue('work_planning_excel');
            dispatch($work_planning_excel);
        }
        catch (\Exception $e)
        {
            \Log::info('work_planning_excel_7');
