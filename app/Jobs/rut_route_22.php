...
class rut_route_22 implements ShouldQueue
{
    ...
    public function handle()
    {
        try
        {
            ...
            $log = [$deterioration->log_text, date('Y-m-d H:i:s') . ': [rut] update route flg to ' . ($deterioration->route_flg + 1)];
            $deterioration->log_text = implode(PHP_EOL , $log);
            $deterioration->route_flg += 1;
            $deterioration->save();

            $rut_31 = (new \App\Jobs\rut_31($this->session_id))->onQueue('deterioration_rut');
            dispatch($rut_31);
        }
        catch (\Exception $e)
        {
            \Log::info('rut_route_22');
    ...
    private function createChartDataPerType($xlsx, $type, $case, $from)
    {
        ...
        $dom->parentNode->removeChild($dom);
        // Add chart $y_axis
        $xml = $this->setYAxisLabel($xml, trans('wp.rutting_depth_mm'));
