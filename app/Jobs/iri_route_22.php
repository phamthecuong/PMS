...
class iri_route_22 implements ShouldQueue
{
    ...
    public function handle()
    {
        try
        {
            ...
            $iri_31 = (new \App\Jobs\iri_31($this->session_id))->onQueue('deterioration_iri');
            dispatch($iri_31);
        }
        catch (\Exception $e)
        {
            \Log::info('iri_route_22');
    ...
    private function createChartDataPerType($xlsx, $type, $case, $from)
    {
        ...
        $dom->parentNode->removeChild($dom);
        // Add chart $y_axis
        $xml = $this->setYAxisLabel($xml, trans('wp.iri_mm_m'));
