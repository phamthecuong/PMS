...
class wp_excel_clist1to5 extends WorkPlanningExcel implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try
        {
            ...
            $xml = $xlsx->arrXMLs['/xl/worksheets/sheet14.xml'];
            ...
            // $dom->parentNode->removeChild($dom);
            // /*** END HERE ***/
            $xml = $this->_writeSheet($this->session_id, 10, $xml, $list, 'en', $xlsx);
            $xlsx->arrXMLs['/xl/worksheets/sheet14.xml'] = $xml;
            ...
            $xlsx = new \eiseXLSX($template);
            $xml = $xlsx->arrXMLs['/xl/worksheets/sheet14.xml'];
            $xml = $this->_writeSheet($this->session_id, 10, $xml, $list, 'vn', $xlsx);
            $xlsx->arrXMLs['/xl/worksheets/sheet14.xml'] = $xml;

            $xlsx->Output("Output_WP_". $list ."_vn.xlsx", "F");
            $work_planning = tblWorkPlanning::findOrFail($this->session_id);
            $work_planning->{"excel_flg_{$list}"} = 13;
            $work_planning->save();
            $work_planning_excel = (new \App\Jobs\wp_excel_remaining_section($this->session_id, $list))->onQueue('work_planning_excel');
            dispatch($work_planning_excel);
        }
        catch (\Exception $e)
        {
            \Log::info('work_planning_excel_14');
