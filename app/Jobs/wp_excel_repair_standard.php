...
class wp_excel_repair_standard extends WorkPlanningExcel implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try
        {
            ...
            $this->_writeRepairStandardSetting($this->session_id, $xlsx, 'en');
            $xlsx->Output("Output_WP_". $list . "_en.xlsx", "F");

            $template = "Output_WP_". $list . "_vn.xlsx";
            $xlsx = new \eiseXLSX($template);
            $this->_writeRepairStandardSetting($this->session_id, $xlsx, 'vn');
            $xlsx->Output("Output_WP_". $list . "_vn.xlsx", "F");

            $work_planning = tblWorkPlanning::findOrFail($this->session_id);
            $work_planning->{"excel_flg_{$list}"} = 3;
            $work_planning->save();
            $work_planning_excel = (new \App\Jobs\wp_excel_rmd($this->session_id, $list))->onQueue('work_planning_excel');
