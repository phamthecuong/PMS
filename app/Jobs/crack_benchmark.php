...
class crack_benchmark implements ShouldQueue
{
    ...
    public function handle()
    {
        try
        {
            ...
            $log = [$deterioration->log_text, date('Y-m-d H:i:s') . ': [crack] update benchmark flg to ' . ($deterioration->benchmark_flg + 1)];
            $deterioration->log_text = implode(PHP_EOL , $log);
            $deterioration->benchmark_flg += 1;
            $deterioration->save();
    
            $crack_pavement_type_phi = (new \App\Jobs\crack_phi($this->session_id))->onQueue('deterioration_crack');
            $this->dispatch($crack_pavement_type_phi);
        }
        catch (\Exception $e)
        {
            \Log::info('crack_benchmark');
