...
class iri_section_31 implements ShouldQueue
{
    ...
    public function handle()
    {
        try
        {
            ...
            $log = [$deterioration->log_text, date('Y-m-d H:i:s') . ': [iri] update section flg to ' . ($deterioration->section_flg + 1)];
            $deterioration->log_text = implode(PHP_EOL , $log);
            $deterioration->section_flg += 1;
            $deterioration->save();

            $iri_32 = (new \App\Jobs\iri_32($this->session_id))->onQueue('deterioration_iri');
            dispatch($iri_32);
        }
        catch (\Exception $e)
        {
            \Log::info('iri_section_31');
